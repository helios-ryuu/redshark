...
| TC-A14 | Upload avatar lỗi mạng            | Tắt wifi → chọn ảnh                                    | Hiển thị lỗi Snackbar, state rollback                         | 🔲          |
| TC-A15 | Upload avatar > 5MB               | Chọn ảnh > 5MB                                         | `UploadAvatarUseCase` trả `ValidationException`, Snackbar      | ✅          |
| TC-A16 | Xem profile user khác             | Điều hướng `profile/{id}` với id khác currentUser      | `ProfileViewScreen`, isOwner=false, không hiện nút Edit       | 🔲          |
| TC-A17 | Edit bio > 280 ký tự              | Nhập bio > 280 ký tự                                   | UI cap 280 + validate, nút Save disabled                      | ✅          |
| TC-A18 | Chọn skills                       | ProfileEditScreen → tap skill chip                     | Toggle selected, `updateProfile` gửi đúng danh sách          | ✅          |
| TC-A19 | Xóa tài khoản                     | Settings → Delete account → confirm                    | Dialog hiện, placeholder — chưa implement xóa thật           | 🔲          |

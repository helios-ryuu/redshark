# RedShark — Tài liệu dự án Android Native Kotlin

Toàn bộ tài liệu kỹ thuật cho dự án **RedShark Android Native (Kotlin)**, sử dụng **Cloud Firestore** và **Cloudflare R2**.

## Mục lục

| File                                                             | Mô tả                                                                 |
|------------------------------------------------------------------|-----------------------------------------------------------------------|
| [PROJECT_CHARTER.md](PROJECT_CHARTER.md)                         | Tuyên ngôn dự án — mục tiêu, phạm vi, các bên liên quan, rủi ro       |
| [STRUCTURE.md](STRUCTURE.md)                                     | Cấu trúc thư mục dự án + naming convention                            |
| [SCHEMA.md](SCHEMA.md)                                           | Lược đồ Cloud Firestore (collections + operations + security rules)   |
| [SECRET.md](SECRET.md)                                           | Danh sách biến cấu hình đang sử dụng                                  |
| [TIMELINE.md](TIMELINE.md)                                       | Tiến độ 9 tuần cho 3 thành viên                                       |
| [WBS.md](WBS.md)                                                 | Cấu trúc phân rã công việc, người phụ trách, phụ thuộc, mốc thời gian |
| [REPORT.md](REPORT.md)                                           | Báo cáo tổng kết                                                      |
| **Kế hoạch theo giai đoạn**                                      |                                                                       |
| [PLAN-1-FOUNDATION.md](PLAN-1-FOUNDATION.md)                     | Giai đoạn 1 — Nền tảng + mẫu kiến trúc                                |
| [PLAN-2-AUTH.md](PLAN-2-AUTH.md)                                 | Giai đoạn 2 — Xác thực + Hồ sơ                                        |
| [PLAN-3-CONTENT.md](PLAN-3-CONTENT.md)                           | Giai đoạn 3 — Ý tưởng + Công việc + Bình luận                         |
| [PLAN-4-INTERACTION.md](PLAN-4-INTERACTION.md)                   | Giai đoạn 4 — Thông báo + Nhắn tin                                    |
| [PLAN-5-FINAL.md](PLAN-5-FINAL.md)                               | Giai đoạn 5 — Kiểm thử + Dọn dẹp + Phát hành                          |
| **Yêu cầu**                                                      |                                                                       |
| [REQUIREMENT-1-FUNCTIONAL.md](REQUIREMENT-1-FUNCTIONAL.md)       | Yêu cầu chức năng                                                     |
| [REQUIREMENT-2-NONFUNCTIONAL.md](REQUIREMENT-2-NONFUNCTIONAL.md) | Yêu cầu phi chức năng                                                 |
| **Quy trình nghiệp vụ**                                          |                                                                       |
| [PROCESS-1-AUTH.md](PROCESS-1-AUTH.md)                           | Luồng xác thực và hồ sơ                                               |
| [PROCESS-2-CONTENT.md](PROCESS-2-CONTENT.md)                     | Luồng ý tưởng, công việc, bình luận                                   |
| [PROCESS-3-INTERACTION.md](PROCESS-3-INTERACTION.md)             | Luồng thông báo và nhắn tin                                           |
| **Danh sách kiểm thử thủ công**                                  |                                                                       |
| [CHECK-1-AUTH.md](CHECK-1-AUTH.md)                               | Ca kiểm thử xác thực và hồ sơ (20 trường hợp)                         |
| [CHECK-2-CONTENT.md](CHECK-2-CONTENT.md)                         | Ca kiểm thử ý tưởng, công việc, bình luận (24 trường hợp)             |
| [CHECK-3-INTERACTION.md](CHECK-3-INTERACTION.md)                 | Ca kiểm thử thông báo và nhắn tin (17 trường hợp)                     |
